...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ============ Load Git Log Graph / Commit Helpers ============
    let gitLogGraphRendered = false;
    let gitLogFetchPromise = null;
    let gitLogCommitsCache = null;

    function getRepoNameFromPath() {
        const pathParts = window.location.pathname.split('/');
        return pathParts[1] || "WhimsicalPuppet";
    }

    function fetchGitLogData() {
        if (gitLogFetchPromise) {
            return gitLogFetchPromise;
        }

        const repoName = getRepoNameFromPath();
        gitLogFetchPromise = fetch(`/${repoName}/git_log`)
            .then(response => response.json())
            .then(data => {
                console.log("[DEBUG] Fetched git commit graph:", data);
                gitLogCommitsCache = Array.isArray(data.commits) ? data.commits : [];
                makeGitCommitListClickable(gitLogCommitsCache);
                return gitLogCommitsCache;
            })
            .catch(err => {
    ...
    function makeGitCommitListClickable(commits) {
        const commitList = document.querySelectorAll('#gitTreeTab li');
        ...
        commits.forEach(commit => {
            ...
            hashToParent.set(commit.hash, parents.length > 0 ? parents[0] : null);
        });

        commitList.forEach(li => {
            if (li.dataset.hashLinked === 'true') return;
            const rawText = li.textContent || '';
            const match = rawText.trim().match(/^([0-9a-f]{7,40})\b/i);
            if (!match) return;

            const hash = match[1];
            const parentHash = hashToParent.has(hash) ? hashToParent.get(hash) : null;
            const baseRev = parentHash || `${hash}^`;
            const diffUrl = `/${repoName}/diff?baseRev=${encodeURIComponent(baseRev)}&compRev=${encodeURIComponent(hash)}`;
            ...
            const trailingText = rawText.slice(rawText.indexOf(hash) + hash.length);
            li.textContent = '';
    ...
    function loadGitLogGraph() {
        fetchGitLogData()
            .then(commits => {
                if (gitLogGraphRendered) return;
                renderGitLogGraph(commits);
                gitLogGraphRendered = true;
            })
            .catch(err => {
                console.error("[ERROR] Unable to render git log graph:", err);
                const container = document.getElementById('gitLogContainer');
                if (container && !container.dataset.errorShown) {
                    container.textContent = 'Unable to load git log graph.';
    ...
    function renderGitLogGraph(commits) {
        ...
        if (!Array.isArray(commits) || commits.length === 0) {
            container.append('div').text('Loading commit history...');
            return;
        }
        ...
        const roots = commits.filter(commit => {
            if (!Array.isArray(commit.parents) || commit.parents.length === 0) return true;
            return !commitMap[commit.parents[0]];
        });
        ...
        function displayCommit(commit, indent) {
            ...
            row.append('a')
                ...
                .attr('class', 'git-commit-link')
                .text(commit.hash);

            row.append('span')
                .text(` - ${commit.author} - ${commit.date} - ${commit.message}`);
    ...
    // Preload git log data so the commit list is clickable immediately.
    fetchGitLogData().catch(err => {
        console.error("[ERROR] Initial git log fetch failed:", err);
    });
                ...
                .then(data => {
                    if (data.error) {
                        ...
                        data.branches.forEach(branch => {
                            ...
                            opt.value = branch;
                            opt.textContent = formatBranchDisplayName(branch) || branch;
