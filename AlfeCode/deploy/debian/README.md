...
If you also want the **local git host server** for demo repos, run:

```bash
sudo ENABLE_LOCAL_GITHOST=true bash /tmp/alfe-ai/AlfeCode/deploy/debian/bootstrap_alfecode_debian.sh
```
...
7. Optionally configures local git host + demo repo (`ENABLE_LOCAL_GITHOST=true`).
...
### 5) Optional: configure local git host server for demo repos

This is needed if you want local demo repositories available via the bundled git host tooling:
