...
describe("Integration tests for server_webserver.js", () => {
    ...
    it("GET /alfe-dev should redirect to /environment/alfe-dev", async () => {
        const response = await axios.get(`${baseUrl}/alfe-dev`, { maxRedirects: 0 }).catch(err => err.response);
        expect(response.status).toBe(302);
        expect(response.headers.location).toBe("/environment/alfe-dev");
    });

    it("GET /environment/alfe-dev should return 200 and list chats", async () => {
        const response = await axios.get(`${baseUrl}/environment/alfe-dev`);
        expect(response.status).toBe(200);
        // Additional assertions can be added here
    });
