...
class Game:
    ...
    def create_board(self) -> List[Property]:
    ...
    def from_dict(cls, data):
        ...
        game = cls([p["name"] for p in data["players"]])
        game.players = []
        for p_data in data["players"]:
            player = Player(p_data["name"], p_data["position"], p_data["money"])
            player.properties = p_data["properties"]
            player.in_jail = p_data["in_jail"]
            player.jail_turns = p_data["jail_turns"]
            player.is_bankrupt = p_data["is_bankrupt"]
            game.players.append(player)
        
        game.current_player_index = data["current_player_index"]
        game.game_over = data["game_over"]
        game.dice_history = data["dice_history"]
        
        # Rebuild board
        game.board = []
        for p_data in data["board"]:
            property = Property(p_data["name"], p_data["price"], p_data["rent"], p_data["color"])
            property.owner = p_data["owner"]
            property.houses = p_data["houses"]
            property.mortgaged = p_data["mortgaged"]
            game.board.append(property)
